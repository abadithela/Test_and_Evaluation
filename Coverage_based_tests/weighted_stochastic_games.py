    ...
    for env_state in range(1,6):
        for end_env_state in T1[env_state-1]:
            for sys_state in range(1,6):
                ...
                weight = 1
                edge = [start_vtx, end_vtx, "ae", weight]
                GEdges.append(edge)
    ...
    for sys_state in range(1,6):
        for end_sys_state in T2[sys_state-1]:
            for env_state in range(1,6):
                ...
                ll = 1  # Lower limit on weight
                ul = 11 # Upper limit on weight
                weight = ra.randint(ll,ul)
                edge = [start_vtx, end_vtx, "as", weight]
                GEdges.append(edge)
    return GVp, GEdges
...
# cost_max: Maximum cost of systme transition. Weighted winning set function ...
...
def synt_winning_set(GVp, GEdges, U, W0, N):
    ...
    for ii in range(1,N+1):
        Pre_ii = pre(GVp, GEdges, Pre_cur, U, 1, 0, 1, cost_max)
...
def pre(GVp, GEdges, W0, U, qual1, qual2, qual3, cost_max):
    if not GVp: # 2-player game winning set
        ...
        for env_win in env_W0:
            ...
            weights = [row[3] for row in GEdges]
            env_nbr = [start_node[ii] for ii in env_win_idx]
            weight_cap = [(weights[ii] <= cost_max) for ii in env_win_idx]
            if env_nbr: # If list is not empty
                for ii in range(0,len(env_nbr)):
                    weight = weight_cap[ii]
                    env_nbr_elem = env_nbr[ii]
                    if ((env_nbr_elem not in U) and weight):  # Not in unsafe set
...
# def edge_info(GEdges, N, sys_win, env_win):
...
#     gfp_sys_win_set = sys_win[N-1] # Largest winning set for system action states
#     gfp_env_win_set = env_win[N-1] # Largest winning set for environment action states
